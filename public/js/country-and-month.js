...
function populateStates( countryElementId, stateElementId ){
	...
	for (var i=0; i<state_arr.length; i++) {
		stateElement.options[stateElement.length] = new Option(state_arr[i],state_arr[i].replace(/[!;.,-@#$%^&*()+"' ]/g,"_"));
	}
}
...
function populateCountries(countryElementId, stateElementId){
	...
	for (var i=0; i<country_arr.length; i++) {
		countryElement.options[countryElement.length] = new Option(country_arr[i],country_arr[i].replace(/[!;.,-@#$%^&*()+"' ]/g,"_"));
	}
...
function expYear(yearID){
	var expYear = document.getElementById(yearID);
	var currentYear = (new Date).getFullYear();
	expYear.length=0;
	expYear.options[0] = new Option('Select Year','');
	expYear.options[1] = new Option(currentYear, currentYear);
	expYear.selectedIndex = 0;
	for (var i=1; i< 10; i++) {
		var nextYear = currentYear + i;
		expYear.options[expYear.length] = new Option(nextYear,nextYear);
	}
}