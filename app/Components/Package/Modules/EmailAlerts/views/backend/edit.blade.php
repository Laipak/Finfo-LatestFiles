...
@section('content')
    <section class="content" id="press-release">
        <div class="row head-search">
            <div class="col-sm-6">
                <lable class="label-title">Edit Email Alert</lable>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                {!! Form::open(array('route' => ['package.admin.email-alerts.post', $email->id], 'id' => 'frm-ir-calendar', 'files'=> true)) !!}
                    ...
                    <div class="form-group">
                            ...
                            {!! Form::text('email_address', $email->email_address, ['class'=>'form-control', 'placeholder'=>'Email Address']) !!}
                    ...
                    <div class="form-group">
                            ...
                            @foreach($modules as $cat)
                                @if($cat->name != 'Email Alerts' && $cat->name != 'Media Access')
                                    <div class="checkbox">
                                       <label>
                                          <input type="checkbox" name="category[]" value="{{$cat->id}}" {{(in_array($cat->id, $arr_category_id))? 'checked': ''}}>{{$cat->name}}
                                       </label>
                                    </div>
                                @endif
                            @endforeach
