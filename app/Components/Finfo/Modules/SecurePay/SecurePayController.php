<?php
namespace App\Components\Finfo\Modules\SecurePay;
...
class SecurePayController extends BaseController
{
	...
	*/
	function TestConnection() {
		...
		$this->ResponseXml = $this->_Dispatch($this->RequestXml);
		$this->ResponseTree = simplexml_load_string($this->ResponseXml);
                //var_dump($this->ResponseTree->Status->statusCode);
	...
	function _Dispatch($xml) {
		...
		if ($this->IsRepeat()) { // Periodic payment
			$url = ($this->TestMode) ? 'https://www.securepay.com.au/test/periodic' : 'https://www.securepay.com.au/xmlapi/periodic';
                    ...
                    // Once-off payment
                    // COMMAN TO RUN ON LOCAL
                    $url = ($this->TestMode) ? 'https://test.securepay.com.au/xmlapi/payment' : 'https://api.securepay.com.au/xmlapi/payment';
                    // UNCOMMAN TO RUN ON PRODUCTION
                    //$url = ($this->TestMode) ? 'http://test.securepay.com.au/xmlapi/payment' : 'http://api.securepay.com.au/xmlapi/payment';
                }
